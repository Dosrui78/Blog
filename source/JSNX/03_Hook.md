...
Hook技术又叫`钩子函数`，在系统没有调用该函数前，钩子程序就先捕获该消息，钩子函数先得到控制权，这时钩子函数既可以加工处理（改变）该函数的执行行为，还可以强制结束消息的传递。

简单来说，就是把**系统的程序**拉出来变成我们自己执行代码片段。

在js中，系统程序可以指`浏览器API`，也可以之代码中实现的一些方法等。
...
![](https://dosrui.oss-cn-guangzhou.aliyuncs.com/img/图片1.jpg)
...
再次hook原型链，会卡在`debugger`处

```javascript
String.prototype.split_bk = function(val){
    debugger;
    console.log(val)
}
```

为了使Hook函数更完整，可以这样写，但是这样会堆栈溢出
...
于是可以这样重写函数，但是这样返回的是`str.split_bk`，而第一句的`split_bk`方法没用上，所以下述方法也是不行的。
...
返回的`split_bk`就是一开始重写定义的`split_bk`
...
>
> `setRequestHeader()`在第一次 调用之后，每次调用时，指定的文本都会附加到现有标题内容的末尾。
>
> 如果没有[`Accept`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Accept)使用此设置`Accept` 标头，则`"*/*"`在[`send()`](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest/send)调用时随请求发送具有该 类型的标头。
>
> 出于安全原因，某些标头只能由用户代理控制。这些头包括[禁止头名称](https://developer.mozilla.org/en-US/docs/Glossary/Forbidden_header_name) 和[禁止响应头名称](https://developer.mozilla.org/en-US/docs/Glossary/Forbidden_response_header_name)。

![](https://dosrui.oss-cn-guangzhou.aliyuncs.com/img/Snipaste_2021-12-16_23-09-27.png)

`setRequestHeader方法`属于`XMLHTTPRequest原型链`下的方法，可以简单重写一下。
